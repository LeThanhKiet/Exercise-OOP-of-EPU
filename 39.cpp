...
bool Nhanvien::operator<(Nhanvien another) { return this->salary() < another.salary(); }

void sortbySalary(Nhanvien *a, int n){
     for(int i = 1; i < n; i++){
          Nhanvien res = *(a + i);
          int pos = i - 1;
          while(pos >= 0 && *(a + pos) < res){
               *(a + pos + 1) = *(a + pos);
               pos--;
          }
          *(a + pos + 1) = res;
...
int main(){
     ...
     cout << "Danh sach " << n << " nhan vien sap xep giam dan theo Luong: " << endl;
     sortbySalary(a, n);
     for (int i = 0; i < n; i++){
          cout << *(a + i);
     }
